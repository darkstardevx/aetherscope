...
fn protocol_color(transport: Option<&TransportHeader>) -> String {
    match transport {
        ...
        Some(TransportHeader::Icmp { .. }) | Some(TransportHeader::Icmpv6 { .. }) => cybercore::palette::orange(),
...
pub fn render(pkt: &ParsedPacket, raw: &[u8], format: OutputFormat, pretty: bool, color: bool) -> String {
    match format {
...
fn render_summary(pkt: &ParsedPacket, color: bool) -> String {
    let (c, r) = if color { (protocol_color(pkt.transport.as_ref()), reset().to_string()) } else { (String::new(), String::new()) };

    let Some(ip) = &pkt.ip else {
        let ethertype = pkt.eth.as_ref().map(|e| format!("ethertype=0x{:04x}", e.ethertype)).unwrap_or_else(|| "non-ethernet".to_string());
        return format!("{c}[unparsed] {ethertype} len={}{r}", pkt.len);
    };

    let body = match &pkt.transport {
        Some(TransportHeader::Tcp { src_port, dst_port, flags }) => {
            format!("{}:{} > {}:{} TCP [{}]", addr(ip, true), src_port, addr(ip, false), dst_port, flags.short())
        }
        Some(TransportHeader::Udp { src_port, dst_port, .. }) => {
            format!("{}:{} > {}:{} UDP", addr(ip, true), src_port, addr(ip, false), dst_port)
        }
        Some(TransportHeader::Icmp { icmp_type, code }) => {
            format!("{} > {} ICMP type={} code={}", ip.src, ip.dst, icmp_type, code)
        }
        Some(TransportHeader::Icmpv6 { icmp_type, code }) => {
            format!("{} > {} ICMPv6 type={} code={}", ip.src, ip.dst, icmp_type, code)
        }
...
fn render_hexdump(pkt: &ParsedPacket, raw: &[u8], color: bool) -> String {
    ...
    for chunk in raw.chunks(16) {
        ...
        let ascii_string: String = chunk.iter().map(|&b| if (32..=126).contains(&b) { b as char } else { '.' }).collect();
